...
import { Settings, Save, Loader2, Edit3, Building, Briefcase, GraduationCap, Link as LinkIcon, BadgeAlert, CheckCircle2, UserCircle, Camera, X } from 'lucide-react';
...
const Profile = () => {
    ...
    const [formData, setFormData] = useState({
        firstName: '', lastName: '', aboutMe: '', skills: '',
        currentCompany: '', currentTitle: '', yearsOfExperience: '', graduationYear: '',
        department: '', grade: '', experience: '', employmentStatus: 'UNEMPLOYED', linkedinUrl: ''
    });
        ...
        const fetchMyProfile = async () => {
            try {
                ...
                setFormData({
                    ...
                    currentCompany: data.currentCompany || '', currentTitle: data.currentTitle || '',
                    yearsOfExperience: data.yearsOfExperience || '', graduationYear: data.graduationYear || '',
                    department: data.department || '', grade: data.grade || '',
                    experience: data.experience || '', employmentStatus: data.employmentStatus || 'UNEMPLOYED',
    ...
    const handlePhotoChange = async (e) => {
        ...
        if (file.size > 2 * 1024 * 1024) {
            setError('Fotoğraf boyutu 2MB\'ı geçemez.');
            return;
        }
        ...
        reader.onloadend = async () => {
            ...
            try {
                await api.post('/users/photo', { photo: base64 });
    ...
    const handleRemovePhoto = async () => {
        setPhotoLoading(true);
        try {
            await api.post('/users/photo', { photo: '' });
        ...
        try {
            if (isAlumni) {
                await api.put('/alumni/update', {
                    ...
                    currentCompany: formData.currentCompany, currentTitle: formData.currentTitle,
                ...
                await api.put('/profiles/student/update-profile', {
                    ...
                    aboutMe: formData.aboutMe,
                    skills: formData.skills ? formData.skills.split(',').map(s => s.trim()) : [],
                    department: formData.department, grade: formData.grade,
                    experience: formData.experience, employmentStatus: formData.employmentStatus,
                    linkedinUrl: formData.linkedinUrl
                });
            }
            ...
            setError('Hesap güncellenirken bir hata oluştu.');
        } finally {
            setLoading(false);
        }
    };

    // Avatar görüntüsü: fotoğraf varsa onu, yoksa ui-avatars kullan
    const avatarSrc = profilePhoto ||
        `https://ui-avatars.com/api/?name=${formData.firstName}+${formData.lastName}&background=ffffff&color=4f46e5`;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">{isAlumni ? 'Mentor' : 'Öğrenci'} Özgeçmişiniz</p>
                    </div>
                </div>
                {!isEditing && (
                    <button onClick={() => setIsEditing(true)}
                        ...
                        <Edit3 className="w-4 h-4 mr-2" /> Profili Düzenle
                    </button>
                )}
            </div>
                ...
                <div className="bg-white/60 backdrop-blur-3xl shadow-xl rounded-3xl p-8 border border-white/40 space-y-8">
                    <div className="flex items-start gap-6 border-b pb-6 border-gray-200/50">
                        ...
                        <div className="relative flex-shrink-0 group">
                            <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-500 p-1 shadow-lg">
                        ...
                        <div className="space-y-1">
                            ...
                            {formData.linkedinUrl && (
                                <a href={formData.linkedinUrl.startsWith('http') ? formData.linkedinUrl : `https://${formData.linkedinUrl}`}
                                    target="_blank" rel="noreferrer"
                                    className="inline-flex items-center text-sm text-blue-500 hover:text-blue-700 mt-2 font-medium">
                                    <LinkIcon className="w-4 h-4 mr-1" /> LinkedIn Profili
                                </a>
                            )}
                            <p className="text-xs text-gray-400 pt-1">Fotoğrafı değiştirmek için avatarın üzerine gelin.</p>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {formData.skills ? formData.skills.split(',').map((skill, i) => (
                                    <span key={i} className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-semibold border border-blue-200">
                                        {skill.trim()}
                                    </span>
                ...
                <form onSubmit={handleSubmit} className="bg-white/60 backdrop-blur-3xl shadow-xl rounded-3xl p-8 border border-gray-200 space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p className="text-xs text-gray-400 mt-1">İsim kayıt sırasında alınır ve değiştirilemez.</p>
                        </div>
                        <div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <textarea name="aboutMe" value={formData.aboutMe} onChange={handleChange} rows="3" className="w-full p-3 rounded-xl border border-gray-300 bg-white/50 focus:ring-2 focus:ring-blue-500 resize-none" placeholder="Kendinizden kısaca bahsedin..." />
                        </div>
                        <div className="md:col-span-2">
                        ...
                        <div className="md:col-span-2 border-t pt-6 mt-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                            {isAlumni ? (
                                <>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-1 flex items-center"><Briefcase className="w-4 h-4 mr-1 text-gray-400" /> Mevcut Pozisyon</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-1 flex items-center"><Building className="w-4 h-4 mr-1 text-gray-400" /> Şirket</label>
                                ...
                                <>
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-1 flex items-center"><GraduationCap className="w-4 h-4 mr-1 text-gray-400" /> Bölüm / Program</label>
                                    ...
                                    <div>
                                        ...
                                        <input name="experience" value={formData.experience} onChange={handleChange} className="w-full p-3 rounded-xl border border-gray-300 bg-white/50 focus:ring-2 focus:ring-blue-500" placeholder="Örn: X Şirketinde Yarı Zamanlı" />
                                    </div>
                                </>
                            )}
                        </div>
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-sm font-semibold text-gray-700 mb-1 flex items-center"><LinkIcon className="w-4 h-4 mr-1 text-blue-500" /> LinkedIn Profil Linki</label>
                    ...
                    <div className="flex gap-4 justify-end pt-4">
                        <button type="button" onClick={() => setIsEditing(false)} className="px-6 py-3 border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition-colors">İptal</button>
